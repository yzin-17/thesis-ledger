...
| `ai-provider-probe-route-parameters` | 让 Provider 用途测试与真实执行的参数一致：单次（`single`）探针目前只消费 Provider 总超时，用途级 / Provider 级 `firstOutputTimeoutMs`、`outputIdleTimeoutMs` 与路由级 `reasoningEffort` 都不参与，导致用途面板的“首输出等待 / 输出空闲等待”对“测试”按钮无效，推理型模型只能靠把 Provider 总超时整体抬高才能测试成功 | `tasks/2026-09-21-ai-provider-model-usage-configuration.md`（2026-09-22 修复记录） | 真实 Provider 用途测试因超时参数不可用而被误判为模型不可用，或产品确认测试必须与流式执行共用同一组超时/推理参数时立项；立项前不得用放宽探针预算掩盖真实执行路径的超时 |
