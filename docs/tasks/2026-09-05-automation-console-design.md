...
- 页面拆 Tab（用户反馈「页面太长，参考风险中心拆分 tab」，三 Tab 分组经用户确认）：页面改为风险中心同款 `TabsList variant="line"` + 本地 state 三个 Tab——「数据源」（提供方配置 + 健康历史）、「自动化」（任务 + 运行历史）、「诊断」（通知失败 + 数据质量问题）；`ProviderHistoryTables` 拆为 `AutomationRunHistoryTable` / `NotificationFailuresTable` / `DataQualityIssuesTable` 三个组件分属各 Tab；DataStateBanner 保留在 Tabs 之上（全局）；提供方面板还原为普通 `panel`（Tab 结构下双线问题不复存在）；Spec 界面打磨「页面接线」已同步。浏览器目检三 Tab 切换、新建入口与编辑器均正常。
- 文案去重（用户反馈）：提供方表格名称单元格下的类型子标签与「能力」列重复（通知 Provider 显示两个「通知」）；改为仅当类型标签未被能力列覆盖时才显示（行情 Provider 的「行情」仍保留），浏览器目检确认。
- 按钮归位（用户反馈「新增或更新 Provider 按钮放到合适的地方」）：Tab 拆分后该按钮仍挂页头但只作用于数据源 Tab，移入提供方面板标题行右侧（与自动化 Tab「新建任务」同款 `panel-heading flex` + `size="sm"` 模式），面板补标题「提供方」与说明；页头操作区仅保留 ghost 刷新按钮；Spec 界面打磨已同步。浏览器目检确认。
- 布局统一（用户反馈）：数据与自动化页 T5 改用标准 page-header 后出现两处间距问题——①页头分隔线下 16px 即首个面板顶线（双线观感、表格贴页头，其他页为 24px 起步）；②面板间完全贴合，与其他页的大块分隔节奏不一致。修复：`ProviderTable` 面板 `mt-6 border-t-0`（消除双线并对齐 24px 起步），全部面板包入 `space-y-6`（24px 间隔 + 面板标题自带 16px 内边距 ≈ 市场页 40px 大块节奏）；`.panel` 顶线规则随之迁入 `@layer base`（未分层样式会压过工具类，致 `border-t-0` 失效——与 `.panel-heading` 迁层同理），迁层前已确认无既有工具类依赖旧行为。DOM 实测：首面板 top 198 与市场页一致、首面板顶线 0px、其余面板分隔线保留。
