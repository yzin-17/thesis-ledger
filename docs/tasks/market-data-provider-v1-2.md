...
关联规格：[市场数据与标的中心 Spec v1.2](../specs/market-data-provider-spec-v1.2.md)

任务标识：`market-data-provider-v1-2`

## 执行约束

- 本文只描述后续实施任务；当前阶段只写入 Spec 和任务文档，不实施任何代码、迁移、配置、Compose、UI 或测试改动。
...
- 只有实现完成且有对应验证证据后才能勾选任务；本轮所有实施任务保持未完成。
...
本轮只创建并核对关联 Spec 与本文任务文档，未实施上述任务。T1-T11 均保持未完成；代码测试、迁移、构建、运行时、真实 Provider smoke 和视觉验收均待后续获得实施授权后执行。

## 实施前检查清单

- [ ] 用户确认开始实施，并确认本 Spec 的 Draft 语义不再需要业务调整。
- [ ] 重新读取三个仓库的 `AGENTS.md`、相关 ADR、Data Contract V1 和本 Spec。
- [ ] 确认当前 Git 工作区、三仓基线和可用的 DSA/数据库/浏览器验证环境。
- [ ] 确认 Control Contract 路由、Schema 字段名和迁移编号在不改变本 Spec 语义的前提下落地。
