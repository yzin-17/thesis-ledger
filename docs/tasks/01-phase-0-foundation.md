...
- [x] T009：Fork daily_stock_analysis 并配置 upstream
  ...
  - 验证证据：已创建 `https://github.com/yzin-17/daily_stock_analysis`，本地 `third_party/daily_stock_analysis` 的 `origin/upstream` 配置、`fetch --tags --prune` 和 `merge-tree` dry-run 均通过，详见 `docs/dsa-upstream.md`。
...
- [x] T013：审计 DSA 行情能力
  ...
  - 验证证据：真实 Fork 镜像已完成 A 股、ETF、基金候选和历史行情调用；基金路由的 `404 not_found` 已作为明确能力边界记录在 `docs/dsa-capability-market.md`。
...
- [x] T014：审计 DSA Portfolio / Risk 能力
  ...
  - 验证证据：真实 Fork 镜像完成建账户→交易→Portfolio Snapshot→Risk 报告链路，结果和复用边界记录在 `docs/dsa-capability-portfolio-risk.md`。
...
- [x] T015：审计 DSA Notification 能力
  ...
  - 验证证据：真实 Fork 镜像通过隔离 Webhook 验证 Feishu interactive/text fallback、失败可重试、成功后的 dedup/cooldown 抑制，详见 `docs/dsa-capability-notification.md`。
...
- [x] T016：审计 DSA AI 与 Tool 能力
  ...
  - 验证证据：真实 Fork 镜像注册 18 个工具并通过 `ToolSurface` 执行 Quote、Portfolio/Risk；结果带审计摘要，超出 `600519` 股票 scope 的请求返回不可重试的 `stock_scope_violation`。正式 Portfolio Tool 薄适配、真实 LLM Provider Contract Test 作为发布前置条件，详见 `docs/dsa-capability-ai.md`。
...
- [x] T017：审计 DSA 筹码实现
  ...
  - 验证证据：固定 `600519` 的真实 Provider 调用已保存 unavailable 原始输出；固定交易日 `2025-06-30` 的 Tushare 形状输入和 `compute_cyq_metrics` 输出已保存为确定性 fixture，字段、算法、Provider 边界和缺失策略详见 `docs/dsa-chip-audit.md`。
...
- [x] T018：完成 Phase 0 一致性 Review
  ...
  - 验证证据：已完成 Spec/ADR/目录/Compose/Fork/能力审计/V1 任务矩阵 Review；职责矩阵、静态搜索、Compose 配置解析、Fork `diff --check` 和未决项 owner 记录于 `docs/reviews/phase-0-review.md`。
