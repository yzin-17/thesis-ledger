...
关联规格：[录入持仓与账户模型重构规格](../specs/position-entry-account-model.md)
...
- [x] T6 重构 Desktop `/position-entry`：账户选择、并列录入方式、账户管理 Sheet、偏好恢复、URL 覆盖、脏状态确认和旧路由重定向。
...
| T6-T7 | Desktop 8 tests + typecheck/build；Mobile 6 tests + typecheck/build；入口按账户模式加载独立持仓和现金，Portfolio/Performance/Risk/AI mode 范围实现 | 通过；需浏览器和真机人工验收 |
...
| T10 | migration matrix 16 migrations；infra contract-matrix（Server 72 tests）与三仓发布顺序 | 通过 |
