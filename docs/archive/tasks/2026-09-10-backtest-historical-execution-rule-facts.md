...
- 完整镜像重建暴露出来源级熔断恢复缺口：V2 Effective Policy 会把 `circuit_open` 目标标记为不可用，运行时此前因此无法在 60 秒冷却期后执行半开探测。现保留已配置且启用的熔断目标作为受控候选；近期熔断仍阻止调用，过期熔断只允许一次半开探测，成功后关闭持久化熔断。Provider runtime 与历史可交易性定向测试共 38 项通过，相关 `flake8` 与 `git diff --check` 通过。
- Provider 路由保持原目标，仅将策略 revision 推进至 26 以重新投影当前运行态；未执行数据库结构 SQL、volume 或业务数据清理。正式 `update.sh all` 完成 DSA、Server、Worker 镜像重建，随后 `update.sh dsa` 将熔断恢复修复写入 DSA 镜像；最终 DSA、Server、Worker、PostgreSQL、Redis 均为 healthy。
- 最终镜像重建后 Run `97400612-1bd9-4310-9b6b-8990c56c984f` 再次收敛为 `succeeded / complete`。`snapshotId=8eee9b49958335f9bd47e92595d865e0b040e1144757d556a329e2768a37f57f`，`resultChecksum=069a9ea706503515`，模型哈希仍为 `0b6ac70a935bf63b1a79b1358ce5853624dce4892d6cc03d26a0565b953a5cdf`；结果仍为 58 个权益点、6 次 fill、3 笔闭合交易、0 个拒单和 0 条警告，期末权益 CNY 988,680.97，总收益率 -1.131903%，最大回撤 -1.349657%。
