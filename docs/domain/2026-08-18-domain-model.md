...
| 实体                       | 责任                                   | 事实级别          | 所有者         |
| -------------------------- | -------------------------------------- | ----------------- | -------------- |
| Account                    | 账户身份、机构、类型、模式、币种与状态 | 事实              | Server         |
...
5. Redis 丢失不影响事实完整性。
6. Account 不保存持仓录入来源；手动或截图来源属于 Ledger 事件或 ImportDraft。
7. 真实账户与影子账户使用独立 Ledger 和估值范围，默认聚合不得混合。

V0.1 的截图直接写 Position 是过渡路径；迁移到 Ledger 前不得作为 V1 完成状态。

## 审计结论

历史 Domain Model 总审计确认：Position、Snapshot 和收益结果只能作为可重建投影；AI、Notification 和 DSA 只能消费 Contract/Tool，不能写入 Ledger。Market 数据是带 Provider、marketTime 和质量状态的外部事实副本，长期存储与完整性由主仓负责。

这部分结论已经合并到当前领域模型；完整的历史对象矩阵和审计原文保留在 [`../archive/domain/2026-08-18-domain-audit.md`](../archive/domain/2026-08-18-domain-audit.md)。
