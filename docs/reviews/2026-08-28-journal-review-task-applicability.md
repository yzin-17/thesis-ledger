...
- 原始投资复盘 Spec 与任务：[`2026-08-25-journal-review-interaction-design.md`](../specs/2026-08-25-journal-review-interaction-design.md)、[`2026-08-25-journal-review-interaction-design.md`](../tasks/2026-08-25-journal-review-interaction-design.md)。
...
原 Spec 第 12、84–105 行把 Ledger 作为 Journal 候选的直接组装入口。当前应改为：Ledger 是经济事实源，Journal 消费 Trade Projection；Trade Projection 再提供完整 Trade、Close Slice、成本分配和证据状态。

### 2. 旧候选不是唯一的“交易”粒度

原任务把每次 SELL 形成的候选近似当作一笔交易。当前必须区分：
...
Trade 读取模型和 Ledger V2 使用十进制字符串。新 Spec 已确认 Journal 主链路迁移到 decimal string DTO，旧 `CompletedTrade<number>` 仅保留为 legacy adapter；舍入、不可安全适配和旧快照兼容由新任务 T1/T3 验证。

### 4. 未知开仓边界已在新 Spec 收敛

Trade Projection 允许 `openedAt = null`。新 Spec 已确认保留 Trade Cycle，不使用 `earliestEvidenceAt`、Baseline 记录时间或服务端时间伪造开仓边界；依赖真实开仓的指标统一返回证据不足。

### 5. “未新增数据库迁移”已是历史表述

原任务最终证据声称没有新增数据库迁移；Trade 系统已经引入物化 Trade、Close Slice、Projection Generation、快照和基线迁移。该结论只能保留为原任务当时的范围记录，不能作为当前系统状态。

## 适用性判定

- 原 8/25 Spec：`历史目标仍有参考价值，但不是当前契约`。
- 原 8/25 任务：`历史任务已完成，不应重新执行；T2/T12 的当前职责已被后续 Trade 任务接管`。
- 继续开发 Journal：`使用 8/26 Trade 与 Journal 迁移 Spec；如需新增行为，创建新的 Journal follow-up 任务，不在旧任务上追加隐含范围`。

## Planning Review 结论

- Spec 覆盖：当前 Trade/Journal 子 Spec 已覆盖 Trade Cycle、Close Slice、基线、快照、legacy 和统计资格；原 8/25 文档存在被替代的候选粒度与数据源表述。
- 占位扫描：本次审查的相关 Spec/任务没有发现 `TBD`、`TODO` 或未定义的实现指令。
- 依赖关系：当前 Journal 依赖 Trade Projection、账本修正和 Baseline 对账；8/26 任务中的 T15/T17 已记录该依赖。
- Blocking Question：无。Decimal 主链路、未知 `openedAt`、对象级 `evidenceFingerprint`、半开时间窗口、统计资格和正式 Snapshot 边界已在新 Spec 中确认。
- 结论：旧任务适用性 Review 已完成；新的 Journal Spec/Task 已单独生成，规划预检通过，可从新任务 T1 开始实施。

## 后续建议

1. 保留原 8/25 Spec 和任务文档作为历史基线；后续实现入口使用新的 [`统一 Trade Projection Spec`](../specs/2026-08-28-journal-review-trade-projection.md) 和 [`实施任务`](../tasks/2026-08-28-journal-review-trade-projection.md)。
2. 按新的 [`统一 Trade Projection Spec`](../specs/2026-08-28-journal-review-trade-projection.md) 与 [`实施任务`](../tasks/2026-08-28-journal-review-trade-projection.md) 从 T1 开始，不重新实现 Trade Projection。
3. 后续 Review 使用当前测试和运行态证据，不再引用原任务中的 74/45/210/83 测试计数作为现状。
