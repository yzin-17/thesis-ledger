...
> 实施 PR：#35、#36、#37  
> 状态：PR #37 收口验证中；仓库能力与部署能力门禁分开记录
...
- 策略风险上下文读取实际账户 Position/Trade 平均成本与持仓周期；Exchange 只消费已闭合的目标 timeframe Bar，CN/HK/US 日线按市场 Calendar 最后一段 session 收盘判断完成时点，Fund 使用已发布 NAV；
...
- Risk Center 可把支持的策略来源成本止损/止盈复制成不再跟随策略版本的独立手工规则；不支持的条件不会伪装成等价手工规则；
...
- Desktop 对比固定展示基准，按 Provider+Model 展示尝试/失败信息，候选展示开发/验证/测试真实指标；采纳正式版本后同时返回现有 RiskApplication 到新版本的实际规则差异，仍需用户显式确认升级；
...
1. 日线盘中数据不会替代上一根已闭合日线，分钟 Bar 也必须达到 timeframe 完成时点；
...
6. AI 创建页补 `maxCost`，比较页补固定基准、测试指标和失败通道；Attempt 读模型返回 AiRun token/cost/duration；
7. 正式采纳返回现有 RiskApplication → 新正式版本 MonitoringPlan Diff；
8. Risk Center 支持将可兼容的策略风险规则复制为独立手工规则；
9. 新增 completed tick、恢复 reconciler、通知状态独立性和发送前门禁等定向测试。

PR #37 的最终 CI 与合并后 main CI 在本文件最终收口时补记；未完成前不写成“已通过”。
...
- PR #37 当前 HEAD 的 quality / contracts-and-guardrails / mobile-android-native 全绿；
- PostgreSQL migration/database smoke 真实通过；
- Spec/Task 与本验证记录不再引用过期的“待确认 PR #35/main CI”状态；
- PR #37 合并后再次确认 `main` push CI 全绿。
