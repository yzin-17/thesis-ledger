...
本报告补齐了 macOS release + 真实 Compose API 的主要页面和关键写入路径，但仍不能单独勾选 T272、T278、T283 或 T294：Windows 安装启动、全量 D1-D10 键盘流程、全新数据库 onboarding 和最终签名/tag/release 仍需目标环境或负责人批准；本版本不纳入高对比度模式优化；iOS Simulator 运行证据已独立归档于 `docs/reviews/evidence/mobile-ios/ios-release-e2e.md`。
