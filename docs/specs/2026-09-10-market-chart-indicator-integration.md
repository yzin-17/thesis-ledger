...
> 2026-09-17 后续范围：重新打开与最新边界更新、双向加载一致性、盘中日线快照以 [行情图表更新 Spec](2026-09-17-market-chart-refresh.md) 为准；对应实施和新增验收由该主题 Task 管理，尚未完成。
...
主图区从左到右按日期绘制，价格轴、成交量轴和指标副图共享 x 轴。K 线要求真实且完整的 open/high/low/close；字段缺失时保留日期但该蜡烛为空。未收盘日线的展示、更新和能力降级遵循 [行情图表更新 Spec](2026-09-17-market-chart-refresh.md)，`unknown` 保持未知状态。收盘线只连接有效价格点，不把缺失点连接成虚假连续走势。成交量使用独立轴，缺失时隐藏柱并在口径区说明。价格标记区分最近完整收盘与未收盘快照，不把未完成状态冒充最终收盘。
...
- 用户拖动图表进入当前首条数据左侧的未加载区域时，每个手势只触发一次历史分页；最新边界探测与同日修订按 [行情图表更新 Spec](2026-09-17-market-chart-refresh.md) 验证；右轴辅助系列末值不与价格混排；底部读数使用稳定槽位且不因日期数值变化换行。
...
- AC20：最新边界以已加载末日为 `start` 发起显式刷新，同时接收新增日期和已有日期修订；请求隔离、冷却、无变化反馈、失败重试及重新打开更新统一按 [行情图表更新 Spec 的 AC1–AC5](2026-09-17-market-chart-refresh.md#验收标准) 验收，由该主题 Task 承担新增实现与运行证据。
