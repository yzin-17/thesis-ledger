...
- 连接测试仍以第一个模型为目标：已知 `mandatory = true` 或明确不支持 `none` 时不得发送 `effort: "none"`；其余情况保留现有低成本测试策略。该规则不新增用户可编辑的推理参数。
...
- AC8：已选模型的非敏感推理能力可随 Provider 配置回读；连接测试遇到已知强制推理或不支持 `none` 的模型时不发送 `effort: "none"`，既有配置无元数据时保持兼容。
