...
- 页面按风险中心样式拆分三个 Tab（2026-09-07 用户确认）：「数据源」（提供方配置 + 健康历史）、「自动化」（任务 + 运行历史）、「诊断」（通知失败 + 数据质量问题）；大标题与刷新入口保持在页头，Tabs 紧随其下；
- 自动化任务表标题行加"新建任务"按钮；
...
- 页面头部从无样式定义的标题容器改为其他模块通用的标准 `page-header` 结构 + 操作区；右上角 ghost 图标刷新按钮（28px 同高、muted 色、hover 浅灰底、18px `RefreshCw`、刷新中旋转并禁用，点击重取 Provider 页查询）；"新增或更新 Provider"主按钮随 Tab 拆分移入数据源 Tab 的提供方面板标题行（与自动化 Tab 的"新建任务"同款模式，2026-09-07 调整）；
