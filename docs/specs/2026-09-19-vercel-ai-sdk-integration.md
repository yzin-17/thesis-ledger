...
- **接入就绪**：指定模型、上游格式、Chat 实现、SDK Provider 实现、配置、生成模式及契约具备对应本地 adapter 契约测试证据，可以进入调用；这不代表真实业务验收通过。
...
保留 Provider ID、models、base URL、密钥、enabled、priority、health、timeout、价格、推理配置及数据库/环境变量优先级。新增用户可配置的 `upstreamFormat`（`chat-completions / responses / anthropic-messages`）与条件字段 `chatImplementation`（`compatible / openai-native`）；后者仅在 Chat Completions 下允许存在，默认 `compatible`。`adapter` 不再由 Desktop 选择或写入新配置。按模型保存的执行配置继续包含生成模式、生成契约和首输出/间隔覆盖值；费用使用 Provider 中用户填写的单价、币种和定价版本，不再要求能力声明来源或上游服务限制。复用现有 Provider JSON 配置；API、环境读取和 Desktop 编辑器使用同一显式契约。
...
旧配置中的 `adapter=openrouter` 与 `adapter=openai-compatible` 只在数据库/环境配置读取边界兼容映射为 `chat-completions + compatible`；新建与更新 API 不再接受客户端直接指定 adapter。旧 OpenRouter 配置若实际保存了厂商扩展字段，服务端同时保留只读迁移标记 `compatibilityExtensionProfile=openrouter-v1`，用于约束后述兼容参数转换；该标记不选择 Provider、不对新客户端开放，也不能因编辑名称、地址或模型而丢失。缺少显式新字段的旧配置只按旧 adapter 字段迁移；URL 和显示名称不得参与运行语义推断，不自动改写已保存密钥或模型列表。保留受信代理、本地服务及自定义 baseUrl 访问策略；不接受请求方任意传入目标 URL。

移除 OpenRouter 专用包前，必须逐项处理当前已使用的差异：`reasoningEffort` 通过锁定版本确认的 compatible provider options 映射；仅带受控 `openrouter-v1` 迁移标记的旧配置可将 `provider.require_parameters` 等兼容参数通过 `transformRequestBody` 白名单转换；usage/cost 仅可通过 `metadataExtractor` 归一化为“已报告、估算、未知”。新建通用兼容连接不因 URL 自动获得厂商扩展。无法证明等价的旧配置标记为“待迁移、不可执行”，不得静默放宽请求或把未知费用写为零。迁移只覆盖项目已经使用的必要能力，不复制 OpenRouter adapter 的完整插件、图片、视频或其他厂商能力。
...
1. 锁定 SDK/adapter/契约对应的本地 HTTP/SSE 契约测试证据。证据随版本发布，由受控发布输入登记，不允许页面伪造测试通过。
2. 有效配置、健康/启用状态、预算与允许路由条件满足当前调用。

已有连接测试只证明连接能力，不自动授予结构化能力或预算授权。接入就绪、真实验证、运行健康分别保存，不把一次 Schema 验证失败直接计为整个 Provider 离线。
...
不在失败后自动切模式、删除参数再调用或换模型。原生模式的能力不按模型名称猜测；兼容服务的受控参数映射属于冻结配置的一部分。[S5] 不将随机 `openrouter/free` 设为默认可靠路由。
...
费用授权信任 Provider 配置中用户填写的费用：输入和输出单价均为数字 `0` 且存在有效三位币种时，可作为零费用路由；正费用路由仍需显式预算授权。未填写、只填写一项或币种无效时保持费用未知并阻断需要费用事实的后续执行，不从外部目录自动推断零费用。Provider 实际报告的费用仍优先于用户配置费率，未知费用不得伪造为零。
...
| maxCost | 字符串 `"0"`，默认只允许 Provider 中明确填写为零费用的路由 |
...
AiProvider 配置/列表 API 增量提供上游格式、条件性 Chat 实现、模型执行配置、就绪状态/阻断原因与三类独立验证状态；内部 Provider 包名只作为服务端诊断事实，不成为客户端可写字段。设置页沿用现有编辑与保存流程，增加上述配置及恢复入口；连接测试结果保持独立标记，不自动切换就绪。
...
| AC34 | 接入就绪与 adapter 证据 | 实际 adapter 本地契约证据加有效配置、健康、预算与允许路由条件；与连接健康及真实验收分离；UI 可解释并更正配置 |
...
- 主要风险：免费模型可用性和 Schema 支持变化；adapter 证据可能与真实端点不一致；旧研究任务无发送证据只能保守未知；配置的零交易成本不代表现实费用。以上均有阻断/标记与独立验收规则，不改变成功标准。
