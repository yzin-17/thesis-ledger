...
市场数据页同时把 Provider 注册表、手动配置状态和路由顺序混在两个不够直观的区域中。现有勾选与上下移动作不能直接表达主、备关系；AKShare 内部使用东方财富、新浪或腾讯通道时，也没有向产品侧传递实际上游来源。完整 Provider 清单扩展到 11 项后，配置、路由和标的目录连续纵向堆叠，页面过长且任务边界不清晰。初版 Tab 化仍把技术版本号作为首屏大指标，并在数据源面板重复展示标题与说明，造成信息权重失衡和列表操作区不整齐。
...
3. 市场数据页列出 DSA 内置的全部 Provider，全部 Provider 均可按自身声明的能力参与 ThesisLedger 路由配置。
4. 将每项路由改成明确的主数据源与可选备用数据源，下拉选项来自当前 Provider 清单中兼容该能力与标的类型的项，并显示其配置与启用状态。
5. 将路由策略、Provider 配置和标的目录按独立任务分区，默认进入路由策略；隐藏对普通用户无决策价值的技术版本号，并收紧 Provider 清单密度。
...
- DSA 的 `DataFetcherManager` 还内置 Tushare、TickFlow、pytdx、BaoStock、Yahoo Finance、Longbridge、Finnhub 和 Alpha Vantage；这些数据源的市场覆盖与凭证要求不同。路由配置继续按能力与标的类型表达，运行时由 Provider 判断是否覆盖具体市场，并在不覆盖时进入备用源。
...
- DSA Provider manifest 同时声明 `markets` 和 `configurationMode`。Provider 页面展示全部 DSA 内置数据源，所有 Provider 均可启停、测试，并按 manifest 声明的能力进入路由下拉框。
- Provider runtime 为 11 个 Provider 建立显式适配器映射；需要凭证的 Provider 从其声明的配置方式读取可用状态，未就绪时保留 Desired Policy，但不进入 Effective Policy 的可执行列表。
...
- 选项包含 manifest 声明支持当前能力/标的类型的全部 Provider；未配置凭证或已停用的 Provider 仍可写入 Desired Policy，并在选项中显示当前状态。
- Effective Policy 继续依据 `configured`、`enabled` 和熔断状态计算实际可执行列表；具体标的不在 Provider 市场覆盖范围内时，本次请求按现有 fallback 语义尝试备用源。
- 已保存但临时健康降级的 Provider 仍保留在路由中，避免健康抖动静默改写用户策略；健康状态在 Provider 清单中单独展示。
- 路由数组继续使用现有顺序契约，`[primary, fallback]` 分别对应主、备。

### 页面信息架构

- 页面标题、刷新动作、错误提示和紧凑状态摘要保持在 Tab 之外，切换任务时始终可见。
- 状态摘要只展示路由同步状态、可用数据源数量和本地标的数量，不展示 Desired Policy revision 或目录 generation。
- 使用三个一级 Tab：“路由策略”“数据源”“标的目录”，默认打开“路由策略”。
- 每个 Tab 只承载对应的现有面板，不复制数据请求、草稿或提交状态；切换 Tab 不丢失尚未提交的 Provider 或路由草稿。
- 数据源面板只保留一层标题和说明；Provider 行使用一致的状态区、能力区与操作区，避免操作按钮随内容宽度漂移。
- Tab 使用项目已有的 shadcn/Base UI 组件，支持键盘切换和明确的选中状态；窄屏时允许 Tab 列表横向滚动，不挤压或换行标签。

## 对外行为或接口变化

- `QuoteV1`、`BarV1` 新增可选 `upstreamSource`，旧客户端可忽略。
- DSA Control Provider manifest 新增可选 `origin`、`upstreamSources`、`markets` 和 `configurationMode`，并返回 DSA 内置 Provider 的完整清单。
...
- Provider 清单包含 DSA 当前内置的 11 个数据源，所有 Provider 均可按其能力进入路由配置。
- 路由 UI 对每一项显示主、备选择器，过滤能力或标的类型不兼容的 Provider，并避免重复选择；未配置或已停用状态不阻止 Desired Policy 配置。
- 市场行情详情能显示腾讯财经，市场数据页不承担缓存状态展示。
- 市场数据页通过三个一级 Tab 分隔路由策略、数据源和标的目录；默认展示路由策略，紧凑状态摘要不随 Tab 切换隐藏，面板草稿状态在切换后保留。
...
- DSA Provider 完整清单、11 个 runtime 适配器与路由候选测试。
...
- 11 个 DSA Provider 均提供启停、测试和移除操作，并至少能进入一个自身声明支持的路由候选集合。
...
- 当前路由契约没有市场维度，因此面向特定市场的 Provider 可能对其他市场标的返回不覆盖；运行时必须把该结果作为 Provider 失败并尝试备用源，页面同时展示市场覆盖以辅助配置。
...
- AC3：Provider 配置列出 DSA 内置的全部 11 个 Provider，展示市场覆盖、配置方式和可用状态；11 个 Provider 均可启停、测试并按自身能力参与路由配置。
- AC4：每个路由项使用主、备下拉框，选项来自能力与标的类型兼容的全部 Provider，并标示未配置或已停用状态；主备不会重复。
...
- AC8：市场数据页使用“路由策略”“数据源”“标的目录”三个一级 Tab；默认显示路由策略，顶部不展示策略或目录技术版本号，数据源面板无重复标题且操作布局一致；切换后只展示对应面板且未提交草稿不丢失，键盘与窄屏交互可用。
