...
export function AiRunList({
  ...
  return (
    <aside
      className="flex min-h-0 flex-col gap-3 rounded-md border bg-card p-4"
      aria-label="研究任务列表"
    >
      <div className="px-1">
      ...
      <Tabs value={filter} onValueChange={(value) => onFilterChange(value as AiRunFilterStatus)}>
        <TabsList variant="line" className="w-full gap-2 overflow-x-auto overflow-y-hidden">
          {filters.map((item) => (
            ...
            return (
              <div key={run.id} role="listitem">
                <button
                  ...
                  className="flex w-full flex-col gap-2 rounded-md border border-transparent p-3 text-left transition-colors hover:bg-muted/60 aria-[current=true]:border-border aria-[current=true]:bg-muted"
                >
                  ...
                  <div className="flex min-w-0 items-center justify-between gap-2 text-xs text-muted-foreground">
                    ...
                    <time className="shrink-0" dateTime={run.updatedAt ?? run.createdAt}>
