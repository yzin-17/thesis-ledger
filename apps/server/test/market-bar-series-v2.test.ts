...
import { DsaMarketBarPolicyPort, DsaMarketBarRemotePort, MarketBarReader, PrismaMarketBarFactStore, barSeriesInputFingerprint } from '../src/market/market-bar-reader.js';
...
describe('MarketBarReader V2', () => {
    ...
    projection: {
      effective: {
        ...
        routeStatus: {
          DAILY_BAR: {
            ETF: {
              eligibleTargets: [{ providerId: 'tencent', upstreamSource: 'tencent', routeIndex: 0 }],
            },
          },
        },
      },
  ...
  it('读取前确认 Server Desired 已同步，并校验 DSA effective revision', async () => {
    const control = { getPolicy: vi.fn(async () => ({ revision: 9, enabled: true, syncState: 'applied' })) };
  ...
  it('v2 symbol 未显式提供 assetType 时使用已有 identity 推断边界', () => {
    ...
    const input = (symbol: string) => (controller as unknown as { input: (value: string, query: Record<string, string | undefined>) => { identity: { assetType: string } } }).input(symbol, {});
  ...
  it('指标请求在发送给 DSA 前补齐并规范化默认参数', async () => {
    ...
    const controller = new MarketV2Controller(
      reader as never,
      { calculateIndicatorsV2 } as never,
    );

    await expect(controller.indicator(
      '510300.SH',
      'MA',
      'ETF',
      '1d',
  ...
  it('limit 只切片，后续不同 limit 复用同一 source fact/view', async () => {
    ...
    const reader = new MarketBarReader(policy as never, remote as never, { write: vi.fn(), read: vi.fn() } as never);
  ...
  it('PostgreSQL read-through 按当前 RouteTarget 顺序命中备用来源并保留 provenance', async () => {
    ...
    const remote = { read: vi.fn(async () => { throw new Error('不应请求远端'); }) };
  ...
  it('指标结果集合必须与请求集合完全一致', async () => {
    const controller = new MarketV2Controller({} as never, {
      calculateIndicatorsV2: vi.fn(async () => ({
        contractVersion: 2,
        engineVersion: 'dsa-indicator-v2',
        inputFingerprint: 'remote-fingerprint',
        results: [{ name: 'MA', parameters: { period: 5 }, inputFingerprint: 'remote-fingerprint', points: [] }],
      })),
    } as never);
    const calculate = (controller as unknown as {
      calculate: (value: BarSeriesV2, requests: Array<{ name: 'MA' | 'MACD' | 'RSI'; parameters: Record<string, number> }>) => Promise<unknown>;
    }).calculate.bind(controller);
    await expect(calculate(series(), [
  ...
  it('分布式锁竞争会等待后 recheck，TTL 覆盖完整回退窗口', async () => {
    ...
    const withLock = (reader as unknown as {
    ...
    const acquiredReader = new MarketBarReader({} as never, {} as never, {} as never, { client: acquiredClient } as never);
    const acquired = (acquiredReader as unknown as {
      withDistributedLock: (key: string, work: () => Promise<string>) => Promise<string>;
    }).withDistributedLock.bind(acquiredReader);
    await acquired('key', async () => 'locked');
    expect(acquiredClient.set).toHaveBeenCalledWith(expect.any(String), expect.any(String), 'PX', 12000, 'NX');
  });
  ...
  it('freshness 区分盘中、收盘后、周末和历史复核窗口', () => {
    ...
    expect(resolveBarFreshUntil({ ...series(), identity: { ...series().identity, adjustment: 'qfq' } }, now, { windowEnd: '2025-01-03' }).getTime()).toBe(
      now.getTime() + 7 * 86_400_000,
    );
    expect(resolveBarFreshUntil({ ...series(), identity: { ...series().identity, adjustment: 'none' } }, now, { windowEnd: '2025-01-03' }).getTime()).toBe(
      now.getTime() + 30 * 86_400_000,
    );
    expect(resolveBarFreshUntil({ ...series(), identity: { ...series().identity, adjustment: 'hfq' } }, now, { windowEnd: '2025-01-03' }).getTime()).toBe(
      now.getTime() + 7 * 86_400_000,
    );
    const current = { ...series(), points: [point('2025-01-03')] };
    expect(resolveBarFreshUntil(current, new Date('2025-01-03T06:00:00.000Z')).toISOString()).toBe('2025-01-06T01:30:00.000Z');
    expect(resolveBarFreshUntil({ ...current, points: [point('2025-01-03', 'incomplete')] }, new Date('2025-01-03T06:00:00.000Z')).getTime()).toBe(
      new Date('2025-01-03T06:00:00.000Z').getTime() + 5 * 60_000,
    );
    expect(resolveBarFreshUntil({ ...current, points: [{ ...point('2025-01-03'), completionStatus: 'unknown' }] }, new Date('2025-01-03T08:00:00.000Z')).getTime()).toBe(
      new Date('2025-01-03T08:00:00.000Z').getTime() + 15 * 60_000,
    );
    const missingTail = { ...current, points: [{ ...point('2025-01-03'), completionStatus: 'unknown' as const }] };
    ...
    expect(resolveBarFreshUntil(missingTail, new Date('2025-01-06T03:00:00.000Z'), { windowEnd: '2025-01-03' }).getTime()).toBe(
      new Date('2025-01-06T03:00:00.000Z').getTime() + 5 * 60_000,
    );
    expect(resolveBarFreshUntil(missingTail, new Date('2025-01-06T08:00:00.000Z'), { windowEnd: '2025-01-03' }).getTime()).toBe(
      new Date('2025-01-06T08:00:00.000Z').getTime() + 15 * 60_000,
    );
    expect(resolveBarFreshUntil({ ...current, points: [point('2025-01-03', 'incomplete')] }, new Date('2025-01-04T04:00:00.000Z')).getTime()).toBe(
      new Date('2025-01-04T04:00:00.000Z').getTime() + 15 * 60_000,
    );
    // 国庆假期前最后一个交易日是 9 月 30 日；周末及整段休市期间都应保持到下一交易日开盘。
    expect(resolveBarFreshUntil({ ...current, points: [point('2026-09-30')] }, new Date('2026-10-04T04:00:00.000Z')).toISOString()).toBe('2026-10-08T01:30:00.000Z');
    expect(resolveBarFreshUntil({ ...current, points: [point('2027-01-04')] }, new Date('2027-01-04T04:00:00.000Z')).getTime()).toBe(
      new Date('2027-01-04T04:00:00.000Z').getTime() + 5 * 60_000,
    );
  });

  it('point-in-time 要求 complete 且 availableAt 不晚于 asOf', async () => {
    const remote = { read: vi.fn(async () => series()) };
    const policy = { read: vi.fn(async () => ({ revision: 7, targets: [{ providerId: 'tencent', upstreamSource: 'tencent', routeIndex: 0 }] })) };
    const reader = new MarketBarReader(policy as never, remote as never, { write: vi.fn(), read: vi.fn() } as never);
    await expect(reader.read({ ...input(), acceptance: 'point-in-time', asOf: '2025-01-03T07:30:00.000Z' })).rejects.toThrow('point-in-time');
    await expect(reader.read({ ...input(), acceptance: 'point-in-time', asOf: '2025-01-03T08:30:00.000Z' })).resolves.toMatchObject({ provenance: { effectivePolicyRevision: 7 } });
  });
  ...
  it('source coverage 的 hasMoreBefore 决定较早起点是否需要读穿', async () => {
    ...
    const client = {
      ...
      marketBarSeriesFact: { findMany: vi.fn(async () => [{
    ...
    await expect(store.read(
      ...
      {
        providerId: 'tencent', upstreamSource: 'tencent', routeIndex: 0,
        effectivePolicyRevision: 7, providerRevision: 'unused',
        fetchedAt: '2025-01-03T08:00:00.000Z', freshUntil: '2099-01-01T00:00:00.000Z',
