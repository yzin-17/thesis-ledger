...
export async function formalizeOptimizationCandidate(
  ...
  sourceMode: 'existing' | 'discovery' = 'existing',
) {
  ...
  const candidateStrategy = strategySchemaV2.parse(candidateVersion.schema);
  ...
  return prisma.$transaction(async (transaction) => {
    if (sourceMode === 'discovery') {
      await transaction.strategy.update({
        where: { id: candidateVersion.strategyId },
        data: {
          status: 'draft',
          name: candidateStrategy.name,
          description: candidateStrategy.description ?? null,
        },
      });
    }
